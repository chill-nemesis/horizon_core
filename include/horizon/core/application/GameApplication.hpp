...
#include "horizon/core/application/BaseLoopApplication.hpp"

#include <horizon/callable/CallbackManager.hpp>
#include <horizon/render/RenderManager.hpp>
#include <horizon/ui/Window.hpp>
#include <memory>
...
namespace HORIZON::CORE::APPLICATION
{
	...
	public:
		...
		/*!
		 * @details Makes sure that all attached elements (ui and render components) are released. After calling this
		 * method, the application will be in a valid state but cannot do anything besides terminating.
		 */
		 ...
		 */
		[[nodiscard]] inline UI::Window& GetWindow() { return _window; }
